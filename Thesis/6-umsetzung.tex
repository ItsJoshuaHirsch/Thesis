...
\todo{bildvergeleich mit ohne autocalendar}
\end{description}
...
Dies geschieht über die Erweiterung der Filterleiste (siehe S. \pageref{par:filter}) um das Dropdown-Filterfenster \glqq Mission-Tags\grqq{} in den entsprechenden Dashboards. \todotext{(Dazu in ref() mehr)}
...
Die Namensgebung resultiert aus der visuellen Anordnung der Datentabellen, welche in Abbildung \todo{Bild Vergleich Star-Snowflake} zu erkennen ist.
...
Jedoch ist dies aufgrund der später entdeckten Funktionalität \glqq Lesezeichen\grqq{} (siehe \ref{par:Lesezeichen}) obsolet.
...
In diesem Beispiel wurde sie dafür verwendet, eine Detail-Tabelle nur dann anzuzeigen, wenn eine Filterung weniger als zehn Einsätze zur Konsequenz hat. (siehe \ref{par:Detail-Tabelle})
...
Dies kann mit den Visualisierungen aus \ref{subsub:zeitpunkt} \todo{evtl bei Dashboards später referenz zu neuen?} bereits weitestgehend beantwortet werden.
...
Des Weiteren wurden Verlinkungen zu gewissen Kennzahlen hinzugefügt, sodass bei einem Bedarf an weiteren Informationen an die entsprechende Stelle geklickt werden kann, um weiterführende Details zu einer bestimmten  \gls{KPI} zu erhalten. \todotext{bild?}
...
detail tabelle
binning von teife ,frequenz etc.
