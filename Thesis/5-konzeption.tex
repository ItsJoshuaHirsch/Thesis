...
Gemäß der Methoden des Requirements Engineering \ref{requireEng} wird anhand der erhobenen und ausgearbeiteten Fragestellungen bzw. Anforderungen aus \ref{4.4 fragestellungen} ein Konzept erarbeitet. 
...
Dabei wurde auf die Demo-Datenbank des internen \gls{ANALYSE} - Servers zurückgegriffen, welche zu diesem Zeitpunkt in etwa 100.000 Missionen enthielt, von denen jedoch grundsätzlich alle nur Test-Missionen sind. 
...
Dabei gibt es die eindeutige \gls{KPI} ''Anzahl Geräte'', welche die distinkten Geräte zählt, von denen bis dato Missionen in dem vorliegenden \gls{ANALYSE}- Server vorliegen.
...
Dabei gibt es beispielsweise die klassische Nachbesprechung zwischen Notfallsanitäter(erklärung Rettungsassistent?) und Auszubildenden oder aber auch die medizinische Forschung, welche Ereignisse und Werte einer Reanimationen untersuchen um gegebenfalls neue Erkenntnisse zu gewinnen.
...
Des Weiteren wurde für die Filterliste vorgeschlagen, nach der jeweiligen ''Device-ID'' filtern zu können.
...
Beispielsweise ''Alle Reanimationen die letzten Monat in der Nacht stattfanden''. 
Qlik Sense bietet eine solche Funktionalität unter dem Namen ''Lesezeichen'', welche getätigte Markierungen und Filter unter einem Namen speichert.
...
Auch Informationen beim ''\gls{Hovern}'' oder als zusätzlicher \gls{Button} wären hilfreich, um gegebenenfalls komplexere Diagramme weiter zu erläutern und die zugrundeliegenden Daten zu beschreiben. 
...
So hat der Morgen fünf, der Mittag sieben und Abends nud Nachts jeweils sechs Stunden.
...
Sollte allerdings eine Reanimation ohne Feedbacksensor und ohne Schockabgabe erfolgen\footnote{Nicht jeder Herzstillstand ist Defibrillationspflichtig \cite{Nolan.2010}}, fällt sie zum derzeitigen Zeitpunkt in die Kategorie ''sonstiger Einsatz''
So wurde über Ereignissen überlegt, welche eine Reanimation kennzeichnen.
Beispiele hierfür waren unter anderem das Öffnen des Defibrialltions-Menüs, Starten einer Analyse, Kleben von den Defibrillations-Elektroden.
...
Bei der Darstellung der Reanimationsqualität, siehe Abbildung \ref{fig:reanimation}, gab es die Vorschläge, nicht jeden einzelnen Wert präzise zu präsentieren, sondern ein ''\gls{Binning}'' durchzuführen.
...
Bei den Schocks sei die jeweilige Impedanz bei den einzelnen Defibrillationen von Relevanz, da ansonsten zu Forschungzwecken die bloße Joule-Angabe wenig nützt.
